...
class ExcelUtility {
  ...
  function ExportDatabase() {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'DRAWING ELIGIBLE');
    $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'DRAWN');
    $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'OPTED INTO VENDOR DRAWING');
    $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'WON VENDOR DRAWING');
    ...
    foreach ($view_export_table as $row) {
      ...
      $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rownum, $row['drawing']);
      $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rownum, $row['drawn']);
      $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rownum, $row['vendordrawing']);
      $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rownum, $row['vendordrawn']);
    ...
    foreach ($files as $file => $array) {
	  ...
	  $company = $row['C'];
          ...
          $result = $f3->get('db')->exec(
            'INSERT INTO guests ("lastname","firstname","company","pre_registration","drawing","drawn","vendordrawing")
              VALUES (:lastname,:firstname,:company,:pre_registration,:drawing,:drawn,:vendordrawing)',
